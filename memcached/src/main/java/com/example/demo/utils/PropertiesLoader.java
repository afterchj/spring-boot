...
public class PropertiesLoader {
    ...
    private Long reloadInterval = 30 * 60 * 1000l;
    ...
    class PropertyReloadMethod implements Runnable {
        @Override
        public void run() {
            while (reload) {
                logger.warn("property reload start.");
                reload();
                try {
                    Thread.sleep(reloadInterval);
                } catch (InterruptedException e) {
                    logger.error("property reload method is interrupted.");
                }
            }
        }
    }
    ...
    public void setReload(boolean reload) {
        if (reload && this.reload) {
            ...
            thread = new Thread(new PropertyReloadMethod());
            thread.setDaemon(true);
            thread.start();
            this.reload = true;
        } else if (!reload && !this.reload) {
            return;
        } else {
            this.reload = reload;
        }
